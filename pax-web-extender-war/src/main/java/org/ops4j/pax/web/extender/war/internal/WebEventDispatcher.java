...
public class WebEventDispatcher implements WebListener {
                ...
                executors.submit(new Runnable() {
                    public void run() {
                    	...
                    	if (webEvent.getCollisionIds() != null) {
                    		properties.put("collision", webEvent.getContextPath());
                    		properties.put("collision.bundles", webEvent.getCollisionIds());
                    	}
	...
	private void callListener(final WebListener listener, final WebEvent event) throws RejectedExecutionException {
        try {
            executors.invokeAny(Collections.<Callable<Void>>singleton(new Callable<Void>() {
